...
function Build-MockAnalysisResult {
    ...
    [pscustomobject]@{
        ...
        platform = Get-StringValue -Data $Payload -Name 'platform' -DefaultValue ''
        content_type = Get-StringValue -Data $Payload -Name 'content_type' -DefaultValue ''
        route = Get-StringValue -Data $Payload -Name 'route' -DefaultValue ''
        capture_id = Get-StringValue -Data $Payload -Name 'capture_id' -DefaultValue ''
        author = Get-StringValue -Data $Payload -Name 'author' -DefaultValue ''
        published_at = Get-StringValue -Data $Payload -Name 'published_at' -DefaultValue ''
        podcast_title = Get-StringValue -Data $Payload -Name 'podcast_title' -DefaultValue ''
        podcast_author = Get-StringValue -Data $Payload -Name 'podcast_author' -DefaultValue ''
        episode_url = Get-StringValue -Data $Payload -Name 'episode_url' -DefaultValue ''
        episode_id = Get-StringValue -Data $Payload -Name 'episode_id' -DefaultValue ''
        duration_seconds = Get-StringValue -Data $Payload -Name 'duration_seconds' -DefaultValue ''
        tags = @(Get-DataValue -Data $Payload -Name 'tags')
        analyzed_at = Get-Date -Format 'yyyy-MM-dd'
        provider = 'mock'
        provider_reported_model = ''
